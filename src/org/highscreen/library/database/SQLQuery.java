...
public enum SQLQuery {
	DROP_BOOKS_AUTHORS("drop table if exists books_authors"), DROP_TAGS(
			"drop table if exists tags;"), DROP_BOOKS(
			"drop table if exists books;"), DROP_AUTHORS(
			"drop table if exists authors;"), CREATE_AUTHORS(
			"CREATE TABLE `authors` (" + "`AuthorId` integer PRIMARY KEY ASC,"
					+ "`FirstName` varchar(99) NOT NULL DEFAULT '',"
					+ "`MiddleName` varchar(99) NOT NULL DEFAULT '',"
					+ "`LastName` varchar(99) NOT NULL DEFAULT '');"), CREATE_BOOKS_AUTHORS(
			"CREATE TABLE `books_authors` ("
					+ "`BookId` integer NOT NULL DEFAULT '0',"
					+ "`AuthorId` integer NOT NULL DEFAULT '0');"), CREATE_BOOKS(
			"CREATE TABLE `books` (" + " `BookId` integer PRIMARY KEY ASC,"
					...
					+ " `SourceId` integer NOT NULL DEFAULT '0');"), INSERT_AUTHORS(
